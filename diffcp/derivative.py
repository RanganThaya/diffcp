...
def derivative_wrapper(
    A, b, c, dx, dy, ds, x, y, s, cone_dict, warm_start, mode, kwargs
):
...
def derivative_batch(
    ...
    **kwargs,
    ...
    args = [
        (A, b, c, dx, dy, ds, x, y, s, cone_dict, warm_start, mode, kwargs)
        for A, b, c, dx, dy, ds, x, y, s, cone_dict, warm_start in zip(
            As, bs, cs, dxs, dys, dss, xs, ys, ss, cone_dicts, warm_starts
        )
    ]
    # os.environ["OMP_NUM_THREADS"] = f"4"
    results = ray.get([derivative_wrapper.remote(*arg) for arg in args])
...
def derivative_internal(
    ...
    **kwargs,
    ...
    dw = -(x @ dx + y @ dy + s @ ds)
    dz = np.concatenate([dx, D_proj_dual_cone.rmatvec(dy + ds) - ds, np.array([dw])])
